...
void EXPECT_SPAN_EQ(std::span<std::string> span,
  ...
  for (size_t i = 0; i < span.size(); ++i, ++it) {
    EXPECT_EQ(span[i], *it)
        << "Span element " << i << " does not match expected value";
  }
}
