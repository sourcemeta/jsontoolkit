...
TEST(JSONSchema_default_walker_2020_12, core_id) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{default_schema_walker("$id", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}
...
TEST(JSONSchema_default_walker_2020_12, core_defs) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{
      default_schema_walker("$defs", VOCABULARIES_2020_12_CORE)};
...
TEST(JSONSchema_default_walker_2020_12, applicator_allOf_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{
      default_schema_walker("allOf", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}
...
TEST(JSONSchema_default_walker_2020_12, applicator_anyOf_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{
      default_schema_walker("anyOf", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}
...
TEST(JSONSchema_default_walker_2020_12, applicator_oneOf_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{
      default_schema_walker("oneOf", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}

TEST(JSONSchema_default_walker_2020_12, applicator_not_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{default_schema_walker("not", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}

TEST(JSONSchema_default_walker_2020_12, applicator_if_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{default_schema_walker("if", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}

TEST(JSONSchema_default_walker_2020_12, applicator_then_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{default_schema_walker("then", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}

TEST(JSONSchema_default_walker_2020_12, applicator_else_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{default_schema_walker("else", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}
...
TEST(JSONSchema_default_walker_2020_12, applicator_items_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{
      default_schema_walker("items", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}
...
TEST(JSONSchema_default_walker_2020_12, validation_type_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{default_schema_walker("type", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}

TEST(JSONSchema_default_walker_2020_12, validation_enum_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{default_schema_walker("enum", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}
...
TEST(JSONSchema_default_walker_2020_12, validation_const_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{
      default_schema_walker("const", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}
...
TEST(JSONSchema_default_walker_2020_12,
  ...
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{
      default_schema_walker("format", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}
...
TEST(JSONSchema_default_walker_2020_12, metadata_title_without_vocabulary) {
  using namespace sourcemeta::jsontoolkit;
  const auto strategy{
      default_schema_walker("title", VOCABULARIES_2020_12_CORE)};
  EXPECT_EQ(strategy, SchemaWalkerStrategy::None);
}
