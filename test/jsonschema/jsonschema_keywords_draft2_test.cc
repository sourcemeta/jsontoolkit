...
#define EXPECT_KEYWORD_STATIC_DRAFT2(keywords, expected_pointer,               \
                                     expected_dependencies_size)               \
  EXPECT_KEYWORD_STATIC(keywords, expected_pointer,                            \
                        "http://json-schema.org/draft-02/hyper-schema#",       \
                        expected_dependencies_size)

TEST(JSONSchema_keywords_draft2, additionalProperties_anonymous) {
  ...
  EXPECT_KEYWORD_STATIC_DRAFT2(result, "#/properties", 0);
  EXPECT_KEYWORD_STATIC_DRAFT2(result, "#/properties/foo/type", 0);
  EXPECT_KEYWORD_STATIC_DRAFT2(result, "#/properties/bar/additionalProperties",
                               0);
  EXPECT_KEYWORD_STATIC_DRAFT2(result, "#/additionalProperties/type", 0);
  EXPECT_KEYWORD_STATIC_DRAFT2(result, "#/additionalProperties", 1);
  EXPECT_KEYWORD_DEPENDENCY(result, "#/additionalProperties", "/properties");
}

TEST(JSONSchema_keywords_draft2, additionalProperties) {
  const auto schema = sourcemeta::jsontoolkit::parse(R"JSON({
  ...
  EXPECT_KEYWORD_STATIC_DRAFT2(result, "https://example.com#/properties", 0);
  EXPECT_KEYWORD_STATIC_DRAFT2(result,
                               "https://example.com#/properties/foo/type", 0);
  EXPECT_KEYWORD_STATIC_DRAFT2(
