...
// More often than not, JSON Pointers will be parsed
// as part of a bigger JSON document, so test that
// path as the main one.
auto parse_pointer(const std::string &input)
    -> sourcemeta::jsontoolkit::Pointer {
  std::stringstream stream;
  stream << "\"" << input << "\"";
  const sourcemeta::jsontoolkit::JSON document =
      sourcemeta::jsontoolkit::parse(stream);
  return sourcemeta::jsontoolkit::to_pointer(document);
}

TEST(JSONPointer_parse, empty_string) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("");
...
TEST(JSONPointer_parse, slash_f) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/f");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, slash_foo) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/foo");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, slash_0) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/0");
  EXPECT_EQ(pointer.size(), 1);
...
TEST(JSONPointer_parse, slash_1) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/1");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_index());
...
TEST(JSONPointer_parse, slash_1234) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/1234");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_index());
...
TEST(JSONPointer_parse, slash_0_slash_1_slash_2) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/0/1/2");
  EXPECT_EQ(pointer.size(), 3);
  EXPECT_TRUE(pointer.at(0).is_index());
  EXPECT_EQ(pointer.at(0).to_index(), 0);
...
TEST(JSONPointer_parse, slash_0_slash_foo_slash_2) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/0/foo/2");
  EXPECT_EQ(pointer.size(), 3);
  EXPECT_TRUE(pointer.at(0).is_index());
  EXPECT_EQ(pointer.at(0).to_index(), 0);
  EXPECT_TRUE(pointer.at(1).is_property());
...
TEST(JSONPointer_parse, slash_0_slash_1234) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/0/1234");
...
TEST(JSONPointer_parse, slash_1000) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/1000");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_index());
...
TEST(JSONPointer_parse, slash_space_integer) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/ 12");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, slash_foo_slash_space_integer) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/foo/ 12");
  EXPECT_EQ(pointer.size(), 2);
  EXPECT_TRUE(pointer.at(0).is_property());
  EXPECT_EQ(pointer.at(0).to_property(), "foo");
  EXPECT_TRUE(pointer.at(1).is_property());
...
TEST(JSONPointer_parse, slash_foo_slash_space_bar_space) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/foo/ bar ");
  EXPECT_EQ(pointer.size(), 2);
  EXPECT_TRUE(pointer.at(0).is_property());
  EXPECT_EQ(pointer.at(0).to_property(), "foo");
  EXPECT_TRUE(pointer.at(1).is_property());
...
TEST(JSONPointer_parse, positive_hex_index) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/0x23");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, slash_00) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/00");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, slash_01) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/01");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, slash_001) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/001");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, slash_0100) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/0100");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, positive_real_number_index) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/1.23");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, index_in_scientific_notation) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/1e1");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, integer_with_space) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/1 1");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, percentage) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/c%d");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, caret) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/e^f");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, pipe) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/g|h");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, backslash_after_letter) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/i\\\\j");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, double_quote_after_letter) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/k\\\"l");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, escaped_slash_after_letter) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/a~1b");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, escaped_slash_with_backslash_after_letter) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/x\\\\/");
  EXPECT_EQ(pointer.size(), 2);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, escaped_tilde_after_letter) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/m~0n");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, tilde_zero_one_after_letter) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/x~01");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, control_backslash_backspace) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/\\b");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, control_backslash_backspace_within_string) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/foo\\bbar");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, control_backslash_formfeed_within_string) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/foo\\fbar");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, control_backslash_carriage) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/\\r");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
...
TEST(JSONPointer_parse, control_backslash_carriage_within_string) {
  const sourcemeta::jsontoolkit::Pointer pointer = parse_pointer("/foo\\rbar");
  EXPECT_EQ(pointer.size(), 1);
  EXPECT_TRUE(pointer.at(0).is_property());
