...
inline auto reset(std::basic_stringstream<JSON::Char> &stream) -> void {
  stream.str("");
  stream.clear();
}

inline auto parse_index(std::basic_stringstream<JSON::Char> &stream,
...
auto parse_pointer(std::basic_istream<JSON::Char> &stream)
