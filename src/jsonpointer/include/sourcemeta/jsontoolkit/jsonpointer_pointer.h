...
/// @ingroup jsonpointer
template <typename Property> class GenericPointer {
public:
  using Token = GenericToken<Property>;
  ...
  GenericPointer(const std::string token) : data{} {
    this->data.push_back(Token{token});
  }
  ...
  /// ```
  auto push_back(const GenericPointer<Property> &other) -> void {
  ...
  /// assert(pointer.at(2).to_property() == "baz");
  /// ```
  auto push_back(GenericPointer<Property> &&other) -> void {
    this->data.reserve(this->data.size() + other.size());
  ...
  /// assert(pointer.at(1).to_property() == "bar");
  /// ```
  [[nodiscard]] auto initial() const -> GenericPointer<Property> {
    assert(!this->empty());
    GenericPointer<Property> result{*this};
    result.pop_back();
    return result;
  }
  ...
  /// ```
  auto concat(const GenericPointer<Property> &other) const
      -> GenericPointer<Property> {
    GenericPointer<Property> result{*this};
  ...
  /// ```
  auto starts_with(const GenericPointer<Property> &other) const -> bool {
  ...
  /// ```
  auto rebase(const GenericPointer<Property> &prefix,
              const GenericPointer<Property> &replacement) const
      -> GenericPointer<Property> {
    ...
    GenericPointer<Property> result{replacement};
  ...
  auto resolve_from(const GenericPointer<Property> &base) const
      -> GenericPointer<Property> {
    typename Container::size_type index{0};
    ...
    auto new_begin{this->data.cbegin()};
    std::advance(new_begin, index);
    GenericPointer<Property> result;
    std::copy(new_begin, this->data.cend(), std::back_inserter(result.data));
    return result;
  }
  ...
  auto
  operator==(const GenericPointer<Property> &other) const noexcept -> bool {
    return this->data == other.data;
  }

  /// Overload to support ordering of JSON Pointers. Typically for sorting
  /// reasons.
  auto operator<(const GenericPointer<Property> &other) const noexcept -> bool {
    return this->data < other.data;
  }
